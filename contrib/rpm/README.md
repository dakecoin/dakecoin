...
The RPM spec file provided here is for DakeCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://DakeCoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DakeCoin git repository](https://github.com/DakeCoin/DakeCoin) but are not part of
...
Currently only `Source10` is used, for the example `DakeCoin.conf` file.
...
in the DakeCoin git repository. At present only an SVG version of the DakeCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the DakeCoin
...
needed when 0.12.1 is released, a proper fix is already in the DakeCoin git
master branch.
...
idea to build DakeCoin Core against a static version of that library compiled
...
The `DakeCoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `DakeCoin-server` package. At this point, most RPM based
...
The `DakeCoin-server` package should have SELinux support. How to properly do
...
DakeCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/DakeCoin/). He has been
packaging DakeCoin for Fedora at least since 2012.
...
4. I renamed the `DakeCoin` package that contains the Qt GUI to `DakeCoin-core` as
that appears to be how the general population refers to it, in contrast to
`DakeCoin-xt` or `DakeCoin-classic`. I wanted to make sure the general population
