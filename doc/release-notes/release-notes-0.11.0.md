DakeCoin Core version 0.11.0 is now available from:

  <https://DakeCoin.org/bin/DakeCoin-core-0.11.0/>
...
For example, add the following to `DakeCoin.conf`:
...
related to the blockchain in the DakeCoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows DakeCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that DakeCoin Core will 
...
For an example script see [DakeCoin-submittx](https://github.com/laanwj/DakeCoin-submittx).
...
- #5334 `cf87536` libDakeCoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new DakeCoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DakeCoinAmountField
- #5753 `0f44672` Add DakeCoin logo to about screen
...
- #5809 `46bfbe7` Add DakeCoin-cli man page
...
- #5849 `41259ca` contrib/init/DakeCoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename DakeCoin-Qt.app to DakeCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for DakeCoin-tx)
